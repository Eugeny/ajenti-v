...
fpm_service_name = 'php8.1-fpm'


@plugin
class PHP81FPMServiceTest (SanityCheck):
    def __init__(self):
        self.type = _('PHP8.1-FPM service')
...
class PHP81FPM (ApplicationGatewayComponent):
    id = 'php8.1-fcgi'
    title = 'PHP 8.1 FastCGI'

    def init(self):
        self.config_file = '/etc/php/8.1/fpm/php-fpm.conf'
    ...
    def __generate_pool(self, location, backend, name):
        ...
        listen = backend.params.get('listen', '/var/run/ajenti-v-php8.1-fcgi-' + name + '.sock') or '/var/run/ajenti-v-php8.1-fcgi-'+ name + '.sock'
    ...
    def create_configuration(self, config):
        ...
        cfg = TEMPLATE_CONFIG_FILE % {
            'pidfile': '/var/run/php/php8.1-fpm.pid',
