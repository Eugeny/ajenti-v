...
@plugin
class PHP72FPMServiceTest (SanityCheck):
    def __init__(self):
        self.type = _('PHP7.2-FPM service')
...
@plugin
class PHP72FPM (ApplicationGatewayComponent):
    id = 'php7.2-fcgi'
    title = 'PHP 7.2 FastCGI'

    def init(self):
        self.config_file = '/etc/php/7.2/fpm/php-fpm.conf'
    ...
    def __generate_pool(self, location, backend, name):
        ...
        listen = backend.params.get('listen', '/var/run/ajenti-v-php7.2-fcgi-' + name + '.sock') or '/var/run/ajenti-v-php7.2-fcgi-'+ name + '.sock'
    ...
    def apply_configuration(self):
        PHP72FPMRestartable.get().schedule()

    def get_checks(self):
        return [PHP72FPMServiceTest.new()]


@plugin
class PHP72FPMRestartable (Restartable):
    def restart(self):
