...
fpm_service_name = 'php8.0-fpm'


@plugin
class PHP81FPMServiceTest (SanityCheck):
    def __init__(self):
        self.type = _('PHP8.0-FPM service')
...
@plugin
class PHP81FPM (ApplicationGatewayComponent):
    id = 'php8.0-fcgi'
    title = 'PHP 8.0 FastCGI'

    def init(self):
        self.config_file = '/etc/php/8.0/fpm/php-fpm.conf'
    ...
    def __generate_pool(self, location, backend, name):
        ...
        listen = backend.params.get('listen', '/var/run/ajenti-v-php8.0-fcgi-' + name + '.sock') or '/var/run/ajenti-v-php8.0-fcgi-'+ name + '.sock'
    ...
    def create_configuration(self, config):
        ...
        cfg = TEMPLATE_CONFIG_FILE % {
            'pidfile': '/var/run/php/php8.0-fpm.pid',
            'pools': '\n'.join(self.__generate_website(_) for _ in config.websites if _.enabled)
        }
        open(self.config_file, 'w').write(cfg)

    def apply_configuration(self):
        PHP81FPMRestartable.get().schedule()

    def get_checks(self):
        return [PHP81FPMServiceTest.new()]


@plugin
class PHP81FPMRestartable (Restartable):
    def restart(self):
